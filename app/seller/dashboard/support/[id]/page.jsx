...
import { ArrowLeft, Loader2, Send, RefreshCw, Paperclip, X, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function SellerTicketDetailPage() {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const { data } = await api.get(`/seller/support/tickets/${id}`);
            setTicket(data);
        } catch (e) {
    ...
    const onPick = async (e) => {
        ...
        if (attachments.length + files.length > MAX_FILES) return fire("error", `Up to ${MAX_FILES} files.`);
    ...
    const removeAttachment = (pid) => setAttachments(prev => prev.filter(a => a.publicId !== pid));
    ...
    const send = async (e) => {
        ...
        setSending(true);
        try {
            await api.post(`/seller/support/tickets/${id}/replies`, { message: msg.trim(), attachments });
            setMsg("");
            setAttachments([]);
            await load();
        ...
        <div className="grid gap-6">
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-3">
                    <Link href="/seller/dashboard/support" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm hover:bg-slate-50">
                    ...
                    <>
                        <div className="mb-4">
                            ...
                            <div className="text-xs text-slate-500">
                                Opened • {new Date(ticket.createdAt).toLocaleString()} • {labelForCategory(ticket.category)}
                            </div>
                        ...
                        <form onSubmit={send} className="mt-6 grid gap-3">
                            <label className="grid gap-1">
                                <span className="text-sm text-slate-600">Your Reply</span>
                                <textarea
                                    value={msg}
                            ...
                            <div className="flex items-center gap-3 flex-wrap">
                                <label className="inline-flex items-center gap-2 text-sm">
                                    <input type="file" multiple accept={ALLOWED_MIME.join(',')} onChange={onPick} className="hidden" disabled={uploading || sending} />
                            ...
                            <AttachmentRow items={attachments} onRemove={removeAttachment} />
            ...
            {toast && (
                <div
                    className={`fixed bottom-6 right-6 z-40 inline-flex items-center gap-2 rounded-xl border px-4 py-3 text-sm shadow ${toast.type === "success" ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-rose-50 border-rose-200 text-rose-800"
                        }`}
    ...
    return (
        <div className={`rounded-xl border p-3 ${isAdmin ? "bg-indigo-50/40 border-indigo-100" : "bg-slate-50 border-slate-100"}`}>
            <div className="flex items-center justify-between">
                <div className="text-xs text-slate-500">
                    {isAdmin ? "Admin" : "You"} • {m?.author?.name || m?.author?.email || "User"}
                </div>
