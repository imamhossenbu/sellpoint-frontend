...
import { RefreshCw, Loader2, LifeBuoy } from "lucide-react";
...
export default function AdminSupportPage() {
    ...
    const status = search.get("status") || "all"; // all|open|pending|resolved|closed
    ...
    const setParam = (key, value) => {
        ...
        router.push(`/admin/dashboard/support?${params.toString()}`);
    };
    ...
    return (
        <div className="grid gap-6">
                    ...
                    <table className="min-w-full text-sm">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={4} className="py-10 text-center text-slate-500">No tickets.</td></tr>
                            ) : rows.map((t) => (
                                <tr key={t._id} className="border-t">
                                    <Td>
                                        <Link
                                            href={`/admin/dashboard/support/${t._id}`}
                                            className="font-medium text-slate-800 hover:underline"
                                        >
                                            {t.subject}
                                        </Link>
                                    </Td>
                                    <Td>{t.sellerName || "Seller"}</Td>
                                    <Td>
            ...
            <p className="text-xs text-slate-500 inline-flex items-center gap-2">
                <LifeBuoy className="h-4 w-4" /> To enable actions (resolve/close), add an admin PATCH endpoint for ticket status.
            </p>
        </div>
    );
}
